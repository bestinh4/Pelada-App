...
const PlayerList: React.FC<PlayerListProps> = ({
  ...
  const [selectedFilter, setSelectedFilter] = useState<
    'all' | 'confirmed' | 'goalkeeper' | 'monthly' | 'guest'
  >('all');
  ...
  /*
   * Normalização defensiva dos campos vindos do Firestore.
   *
   * Alguns registros antigos podem não possuir determinados campos.
   * Nunca devemos chamar .toLowerCase() diretamente em um valor
   * que pode ser undefined ou null.
   */
  ...
  const getPlayerType = (player: Player) =>
    typeof player.playerType === 'string'
      ? player.playerType
      : 'avulso';
  ...
  const getPlayerPhoto = (player: Player) =>
    typeof player.photoUrl === 'string' && player.photoUrl.trim()
      ...
      : `https://ui-avatars.com/api/?name=${encodeURIComponent(
          getPlayerName(player) || 'Atleta'
        )}&background=003a75&color=fff&size=150`;
  ...
  const adminUser = players.find((p) =>
    ...
    (
      ...
      p.email.toLowerCase() === String(currentUser.email).toLowerCase()
    )
  );
  ...
  const openEditModal = (player: Player) => {
    ...
    setStatsData({
      ...
      position: getPlayerPosition(player) || 'Atacante',
      ...
      role:
        getPlayerRole(player) === 'admin'
          ...
          : 'player',
      playerType:
        getPlayerType(player) === 'mensalista'
          ? 'mensalista'
          : 'avulso',
      status:
        getPlayerStatus(player) === 'presente'
          ? 'presente'
          : getPlayerStatus(player) === 'ausente'
            ? 'ausente'
            : 'pendente',
  ...
  const handleQuickChangePosition = async (
    playerId: string,
    newPosition: string
  ) => {
    if (!isCurrentUserAdmin) return;
  ...
  // 5 equipes oficiais da pelada:
  // 30 atletas de linha + 4 goleiros.
  const fieldSlots =
    match?.fieldSlots && match.fieldSlots === 30
      ? match.fieldSlots
      : 30;
  ...
  // Garantir que as vagas permaneçam em 30 linha + 4 goleiros.
  useEffect(() => {
    if (
      match?.id &&
      (match.gkSlots !== 4 || match.fieldSlots !== 30)
    ) {
  ...
  // Trava de rolagem quando o modal estiver aberto.
  useEffect(() => {
  ...
  // Jogadores presentes ordenados pela confirmação.
  const sortedPresent = [...players]
    .filter((p) => getPlayerStatus(p) === 'presente')
    .sort((a, b) => {
  ...
  sortedPresent.forEach((p) => {
    ...
    // Atletas que faltaram na pelada anterior entram como suplentes.
   ...
   *
   * Agora os campos são convertidos com segurança para string.
   */
  ...
  const filteredPlayers = players.filter((p) => {
    const playerName = getPlayerName(p).toLowerCase();
    const playerPosition = getPlayerPosition(p).toLowerCase();

    const matchesSearch =
      playerName.includes(normalizedSearchQuery) ||
      playerPosition.includes(normalizedSearchQuery);

    if (!matchesSearch) return false;

    if (selectedFilter === 'confirmed') {
      return getPlayerStatus(p) === 'presente';
    }

    if (selectedFilter === 'goalkeeper') {
      return getPlayerPosition(p) === 'Goleiro';
    }

    if (selectedFilter === 'monthly') {
      return getPlayerType(p) === 'mensalista';
    }

    if (selectedFilter === 'guest') {
      return getPlayerType(p) === 'avulso';
    }
  ...
  const handleShareRoster = () => {
    ...
    text += `🏟️ ${
      match?.location || 'Granja Cantinho do Céu'
    }\n\n`;
    ...
    confirmedGKs.forEach((p, idx) => {
      const isMensalista =
        getPlayerType(p) === 'mensalista';

      const tag = isMensalista
        ? ' [MENSALISTA]'
        : '';

      text += `${String(idx + 1).padStart(2, '0')}. ${
        getPlayerName(p) || 'Atleta'
      } (GK) [${Number(p.goals) || 0}G]${tag}\n`;
    });
    ...
    confirmedField.forEach((p, idx) => {
      ...
      text += `${String(
        confirmedGKs.length + idx + 1
      ).padStart(2, '0')}. ${
        getPlayerName(p) || 'Atleta'
      } [${Number(p.goals) || 0}G]${tag}\n`;
    });
    ...
    if (waitingList.length > 0) {
      ...
      waitingList.forEach((p, idx) => {
        text += `#${idx + 1}. ${
          getPlayerName(p) || 'Atleta'
        } (${getPlayerPosition(p) || 'Sem posição'})\n`;
      });
    }
  ...
  const handleDeletePlayer = async (player: Player) => {
    ...
    if (
      !confirm(
        `Remover atleta "${getPlayerName(player) || 'Atleta'}" permanentemente?`
      )
    ) {
      return;
    }
  ...
  const handleSaveStats = async () => {
    ...
    try {
      const isNowPresent =
        statsData.status === 'presente';

      await updateDoc(
        doc(
          db,
          'players',
          selectedPlayerForStats.id
        ),
        {
          name:
            statsData.name.trim() ||
            getPlayerName(selectedPlayerForStats) ||
            'Atleta',

          position:
            statsData.position || 'Atacante',
          ...
          goals: Math.max(
            0,
            Number(statsData.goals) || 0
          ),

          suplenteNextMatch:
            statsData.suplenteNextMatch,

          confirmedAt: isNowPresent
            ? (
                selectedPlayerForStats.confirmedAt ||
                new Date().toISOString()
              )
            : null
        }
      ...
      alert(
        'Erro ao salvar alterações do atleta.'
      );
    } finally {
              ...
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-headline-sm text-headline-sm text-navy-deep font-bold">
                    {match
                      ? `Pelada • ${match.time || '20h00'}`
                      : 'Pelada de Sábado • 20h00'}
                  </span>
                </div>
            ...
            <div className="w-full h-2.5 bg-surface-container rounded-full overflow-hidden p-0.5">
              ...
              <div
                ...
                style={{
                  width: `${Math.min(
                    100,
                    (confirmed.length / totalSlots) * 100
                  )}%`
                }}
              ></div>
      ...
      >
        {filteredPlayers.length > 0 ? (
          ...
          filteredPlayers.map((player, idx) => {
            ...
            const jerseyFormatted =
              String(jerseyNum).padStart(2, '0');
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div
                      className={`relative w-12 h-12 rounded-full overflow-hidden shrink-0 shadow-sm ${
                        isAbsent
                          ? 'grayscale'
                          : ''
                      }`}
                    ...
                    <div className="flex flex-col min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        <span
                          className={`font-headline-sm text-headline-sm text-navy-deep break-words ${
                            isAbsent
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {isCurrentUserAdmin ? (
                          ...
                          <span className="font-semibold text-navy-deep text-body-sm">
                            {position ||
                              'Posição não cadastrada'}
                          </span>
                    ...
                    <button
                      ...
                      className={`text-[11px] font-bold flex items-center gap-1 active:scale-95 transition-transform ${
                        player.suplenteNextMatch
                          ...
                          : 'text-outline hover:text-amber-700'
                      }`}
      ...
      <div className="sticky bottom-2 w-full pt-2 z-40">
        ...
        >
          ...
          <span className="tracking-wide">
            {copiedFeedback
              ? 'COPIADO COM SUCESSO! ABRINDO WHATSAPP...'
              : 'ZAP DA LISTA • COPIAR ESCALAÇÃO'}
          </span>
              ...
              <div className="flex flex-col gap-3.5 overflow-y-auto pr-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <label className="font-label-md text-xs sm:text-sm text-outline font-semibold block mb-1">
                      Categoria
                    </label>

                    <select
                      value={
                        statsData.playerType
                      }
                      onChange={(e) =>
                        setStatsData({
                          ...statsData,
