...
const TeamBalancing: React.FC<TeamBalancingProps> = ({ players, onPageChange }) => {
  ...
  const [outfieldLimit, setOutfieldLimit] = useState<number>(6);
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "sessions", "current"), (snap) => {
      if (snap.exists()) {
        const sessionData = snap.data() as MatchSession;
        setSession(sessionData);
        if (sessionData.outfieldLimit) {
          setOutfieldLimit(sessionData.outfieldLimit);
        }
      } else {
        setSession(null);
      }
  ...
  const handleGenerateNormal = async () => {
    ...
    let numTeams = Math.ceil(totalFieldPlayers / outfieldLimit);
    ...
    // Atualizar isIncomplete based on outfieldLimit + GK
    teams.forEach(t => {
      const targetSize = outfieldLimit + (t.hasGoalkeeper ? 1 : 0);
      t.isIncomplete = t.playerIds.length < targetSize;
    });
              ...
              <div className="lg:col-span-5 space-y-8">
                <div className="bg-white border border-slate-100 rounded-[3rem] p-10 text-navy relative overflow-hidden shadow-elite min-h-[260px] flex flex-col justify-center">
                   ...
                   <div className="relative z-10">
                    ...
                    <h3 className="text-4xl font-condensed italic font-black mb-6 tracking-tight">{selectedIds.size} ATLETAS CONFIRMADOS</h3>
                    <div className="mb-6">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2">SISTEMA DE JOGO</p>
                      <div className="grid grid-cols-2 gap-3">
                        <button
                          onClick={() => setOutfieldLimit(6)}
                          className={`py-3 px-2 rounded-2xl border-2 font-black uppercase text-[10px] tracking-wider transition-all flex flex-col items-center justify-center gap-1 ${
                            outfieldLimit === 6 
                              ? 'bg-navy border-navy text-white shadow-lg' 
                              : 'bg-slate-50 border-slate-100 text-slate-400 hover:bg-slate-100/50'
                          }`}
                        >
                          <span className="text-sm font-black italic">6 NA LINHA</span>
                          <span className="text-[8px] opacity-75">1 no gol (7 vs 7)</span>
                        </button>
                        <button
                          onClick={() => setOutfieldLimit(7)}
                          className={`py-3 px-2 rounded-2xl border-2 font-black uppercase text-[10px] tracking-wider transition-all flex flex-col items-center justify-center gap-1 ${
                            outfieldLimit === 7 
                              ? 'bg-navy border-navy text-white shadow-lg' 
                              : 'bg-slate-50 border-slate-100 text-slate-400 hover:bg-slate-100/50'
                        ...
                        >
                          <span className="text-sm font-black italic">7 NA LINHA</span>
                           <span className="text-[8px] opacity-75">1 no gol (8 vs 8)</span>
                        </button>
                      </div>
                    </div>
              ...
              <div className="lg:col-span-8 space-y-8">
                <div className="flex items-center justify-between px-2">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                    ...
                    <span className="text-[9px] font-black px-2 py-0.5 bg-navy/5 text-navy rounded-md uppercase tracking-wider self-start sm:self-center">
                      {session.outfieldLimit || outfieldLimit} NA LINHA + 1 NO GOL
                    </span>
                  </div>
