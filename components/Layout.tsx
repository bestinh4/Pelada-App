  ...
  return (
    <div className="flex justify-center min-h-screen sm:py-8 bg-champions-gradient font-display">
      <div className="relative w-full sm:max-w-[430px] sm:h-[880px] bg-transparent sm:rounded-[3.5rem] overflow-hidden flex flex-col transition-all duration-1000">
        ...
        {/* MENU INFERIOR CRYSTAL GLASS */}
        <div className="absolute bottom-8 left-6 right-6 z-[100] animate-slide-up">
          <nav className="h-20 glass-surface rounded-[2.5rem] flex items-center justify-between px-3">
            ...
            {isAdmin && (
              <div className="flex items-center gap-2 px-1">
                <button 
                  ...
                  className={`w-13 h-13 rounded-[1.25rem] flex items-center justify-center transition-all duration-500 shadow-xl active:scale-90 ${currentPage === Page.ArenaPanel ? 'bg-primary text-white scale-110 shadow-primary/30' : 'bg-white/60 text-navy border border-white/80'}`}
                  title="Painel Live"
                >
...
const NavItem = ({ icon, active, onClick, label }: { icon: string, active: boolean, onClick: () => void, label: string }) => (
  <button 
    onClick={onClick}
    className={`flex flex-col items-center justify-center w-12 h-full transition-all duration-500 relative ${active ? 'text-navy' : 'text-slate-400'} group`}
  >
    ...
    {active && (
      <span className="absolute bottom-1 w-1 h-1 bg-primary rounded-full shadow-[0_0_12px_rgba(200,16,46,0.6)] animate-pulse"></span>
    )}
  </button>
);
